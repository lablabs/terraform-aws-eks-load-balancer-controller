...
moved {
  ...
  to   = aws_eks_pod_identity_association.pod_identity[0]
}

moved {
  from = aws_iam_policy.eks_pod_identity[0]
  to   = aws_iam_policy.pod_identity[0]
}
...
moved {
  ...
  to   = aws_iam_role.pod_identity[0]
}

moved {
  from = aws_iam_role_policy_attachment.eks_pod_identity[0]
  to   = aws_iam_role_policy_attachment.pod_identity[0]
}
